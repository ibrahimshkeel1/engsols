        ...
        {(["all", "upcoming", "live", "ended"] as const).map((t) => (
          <button
            ...
            className={cn(
              ...
              tab === t ? "bg-zone-live text-white shadow-zone-live" : "bg-muted text-muted-foreground hover:text-foreground",
            )}
          >
            {t}
          </button>
            ...
            return (
              ...
              >
                <div className="relative aspect-video bg-muted">
                  <Image src={videoThumbnail(stream.title.slice(0, 20))} alt="" fill className="object-cover opacity-90 transition group-hover:opacity-100" unoptimized />
                  <div className={cn("absolute left-0 top-0 h-1 w-full", stripe)} />
                  {stream.status === "live" && (
                    <span className="absolute left-3 top-3 flex items-center gap-1.5 rounded-lg bg-zone-live px-2.5 py-1 text-xs font-bold text-white shadow-zone-live">
                      <span className="live-dot h-1.5 w-1.5 rounded-full bg-bg-surface" />
                      LIVE
                    </span>
                  )}
